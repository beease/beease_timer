...
import { asyncFunctionErrorCatcher, functionErrorCatcher } from '../utils/errorHandler';
import argon2 from 'argon2';

const prisma  = new PrismaClient();

export const getTokenByCredential = async (password:string,email:string) => {
...
export const verifyAccessToken = async (accessToken:string) => {
    const payload:PayloadOnAuthJWT | null = functionErrorCatcher(()=> verifyJwt(accessToken))
