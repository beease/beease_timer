...
export const sendInvitationToWorkspace = async (invitedMail:string,workspace:Workspace,inviterUser:clientUser,invitedUser?:clientUser | null)=>{
    let invitationURL 
    if(invitedUser){
        invitationURL = `${process.env.SERVER_URL}/api/invitation.acceptInvitation?invitationToken=${signInvitationJwt(workspace.id,inviterUser.id,invitedUser?.id)}`
    }else{
        invitationURL = `${process.env.SERVER_URL}/api/invitation.signInvitationPage?invitationToken=${signInvitationJwt(workspace.id,inviterUser.id)}`
    }
